...
class FilteringResultsView(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if filtering_result_id:
            result_map = FilteringResultProductMap.objects.filter(filtering_result_id=filtering_result_id)
            teas = []
            for result in result_map:
                tea_id = result.tea_id
                tea = Teas.objects.get(id=tea_id)
                teas.append({"id": tea.id, "name": tea.name, "brand": tea.brand, 'type': tea.type, 'flavor': tea.flavor, 'efficacies': tea.efficacies, 'image_url': tea.image_url, 'site_url': tea.site_url, 'price': tea.price, 'stock': tea.stock, 'create_date': tea.create_date, 'update_date': tea.update_date})
